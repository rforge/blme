...
loadCovarianceDefaults <- function(env) {
  ...
  env$defaultSpectralCommonScale <- COMMON_SCALE_TRUE_NAME;
  env$defaultSpectralDataScale <- FREE_SCALE_NAME;
  env$defaultSpectralFamily <- GAMMA_FAMILY_NAME;
  env$defaultSpectralGammaShape <- expression(ifelse(posteriorScale == SD_SCALE_NAME, 2.0, 1.5));
  ...
  env$defaultCorrelationCommonScale <- COMMON_SCALE_TRUE_NAME;
  ...
  env$defaultCorrelationGammaShape <- expression(ifelse(posteriorScale == SD_SCALE_NAME, 2.0, 1.5));
  ...
  env$defaultCorrelationInverseGammaScale <-
    expression(ifelse(posteriorScale == SD_SCALE_NAME, 10^1, 10^2) *
               (shape + 1));
  env$defaultCorrelationWishartDegreesOfFreedom <- expression(factorDimension + 2);
