cat("\n\nRUnit test cases for blme.0::bglmer function\n\n");
...
test.blme.bglmer <- function()
{
  ...
  checkEquals(blme.0:::getPriorPenalty(bglmerFit), 0);
  
  bglmerFit <- blme.0:::setPrior(bglmerFit, cov.prior="gamma(2, 1, posterior.scale = 'sd')");
  priorPenalty <- -2 * dgamma(bglmerFit@ST[[1]][1], 2, 1, log=TRUE);
  checkEquals(blme.0:::getPriorPenalty(bglmerFit), priorPenalty);
  
  bglmerFit <- blme.0:::setPrior(bglmerFit, cov.prior=NULL,
  ...
  checkWarning(blme.0:::parsePrior(bglmerFit, fixef.prior="normal(common.scale = 'true')"));
}
